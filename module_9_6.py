def all_variants(text: str):
    """
    Генератор подстрок вместо подпоследовательностей.
    Несмотря на задание, выводятся подстроки вместо подпоследовательностей по требованию преподавателя.
    :param text: строка
    :return: объект-генератор, при каждой итерации которого будет возвращаться подстроки переданной строки
    """
...
def test():
    """
    Тест противоречит заданию. Требуется вывести подпоследовательности, в тестовом примере - подстроки.
    """
    a = all_variants("abc")
    for i in a:
        print(i)
